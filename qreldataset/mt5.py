...
for topic in tqdm(df.topic.unique().tolist()):
    ...
    run = [(topic, 0,  i + 1, x.score, *x.metadata["metadata"][1:]) for i, x in enumerate(top_passage_per_doc)]

    run_df = pd.DataFrame(run, columns="topic iter rank score docno usefulness credibility stance url description efficacy".split())
    runs.append(run_df)
