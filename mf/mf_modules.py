...
class AbstractModel(pl.LightningModule):
    def step(self, batch, batch_nb, prefix='train', add_reg=True):
        ...
        return {f'{prefix}_loss': loss, 'loss': loss, 'log': log, 'pred':prediction, 'target':target}
    ...
    def validation_epoch_end(self, outputs):
        ...
        acc_mean = torch.cat([x['pred'].gt(0).eq(x['target'].gt(0)).float() for x in outputs]).mean()
        self.log("val_acc", acc_mean, prog_bar=True)
        log = {'val_loss': loss_mean, 'acc_mean': acc_mean}
        return {'avg_val_loss': loss_mean, 'log': log, 'val_acc_mean': acc_mean}


    def training_epoch_end(self, outputs):
        acc_mean = torch.cat([x['pred'].gt(0).eq(x['target'].gt(0)).float() for x in outputs]).mean()
        self.log("train_acc", acc_mean, prog_bar=True)
        return None
