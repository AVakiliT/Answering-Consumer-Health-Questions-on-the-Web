...
df["text_in"] = df.progress_apply(
    lambda
        x: f"{x.host} {'[HON]' if x.honcode else ''} {' '.join([sent for sent, score in zip(x.sentences, x.sentence_scores) if score > 0.75])}",
...
Path(f'checkpoints/transformer-agg_RW-train_k={batch_size}').mkdir(parents=True, exist_ok=True)
best_dev_acc = 0
    ...
    for n, dl_e in [('DEV', dl_v), ('TST', dl_t)]:
        ...
        if n == 'DEV' and i_epoch >= 7:
            if np.mean(accs) >= best_dev_acc:
                ...
                torch.save(model.state_dict(), f"checkpoints/transformer-agg_RW-train_k={batch_size}/best")

model.load_state_dict(torch.load(f"checkpoints/transformer-agg_RW-train_k={batch_size}/best"))
# model = AutoModelForSequenceClassification.from_pretrained(MODEL_START_POINT,
#                                                            num_labels=3,
#                                                            # output_hidden_states=True
#                                                            ).cuda()
...
run.to_csv(f"runs/WatS-Bigbird2_{int(THRESHOLD*100)}-MT5-TAgg.all", index=False, header=False)
